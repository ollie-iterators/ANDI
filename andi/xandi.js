//=========================================//
//xANDI: data table headers ANDI           //
//Created By Social Security Administration//
//=========================================//

function init_module(){

var xANDIVersionNumber = "11.2.1";

//create xANDI instance
var xANDI = new AndiModule(xANDIVersionNumber,"x");

//Delimeter for the the header cells
xANDI.associatedHeaderCellsDelimeter = " <span aria-hidden='true'>|</span> ";
...
$("#ANDI508-button-prevElement").off("click").click(function(){
    ...
    if(isNaN(index)){ //no active element yet
        ...
        if(xANDI.dataTables.list.length <= 1)
...
xANDI.analyze = function(objectClass){
    ...
    $(TestPageData.allElements).each(function(){
        ...
        //Determine if this is a refresh of xANDI (there is an active element)
    ...
    if(objectClass.elementNums[0] > 0){
        ...
        .click(function(){
            ...
            andiFocuser.focusByIndex(1);
            xANDI.redoMarkup();
            xANDI.viewList_highlightSelectedTable(activeTableIndex, true);
            andiResetter.resizeHeights();
            return false;
        })
        .mousedown(function(){
...
AndiModule.inspect = function(element){
    if ($(element).hasClass("ANDI508-element")) {
        ...
        //Remove other xANDI highlights
        $("#ANDI508-testPage .xANDI508-highlight").removeClass("xANDI508-highlight");
        //Highlight This Element
        $(element).addClass("xANDI508-highlight");
    ...
    function grabHeadersAndHighlightRelatedCells(element){
        ...
        if(!AndiModule.activeActionButtons.scopeMode){
            ...
            if($(element).is("th")){
                ...
                if(id){
                    $(table).find("th.ANDI508-element:not(.xANDI508-highlight),td.ANDI508-element:not(.xANDI508-highlight)").each(function(){
        ...
        function addHighlight(element, updateAssociatedHeaderCellsText){
            ...
            if(updateAssociatedHeaderCellsText){
                ...
                if(accumulatedHeaderTextArray.indexOf(text) === -1){
                    ...
                    accumulatedHeaderText += text + xANDI.associatedHeaderCellsDelimeter;
                }
            }
        }
...
//This function will remove xANDI markup from every table and rebuild the alert list
xANDI.reset = function(){
    var testPage = document.getElementById("ANDI508-testPage");

    //Every ANDI508-element
    $(testPage).find(".ANDI508-element").each(function(){
        $(this)
            .removeClass("xANDI508-highlight")
...
xANDI.hideModeButtons = function(){
...
xANDI.showModeButtons = function(mode){
    AndiModule.activeActionButtons.modeButtonsVisible = true;
...
function analyzeTable(table){
    ...
    if(role === "table" || ((role === "grid" || role === "treegrid") && $(table).find("[role=gridcell]").first().length)){
        ...
        //The way xANDI analyzes the table is that it begins looking at the cells first
        ...
        $(all_cells).each(function loopC(){
            ...
            if(headers)
                xANDI.grab_headers(cell, andiData, table);
        ...
        if(tableHasHeaders){
            ...
            if(AndiModule.activeActionButtons.modeButtonsVisible && $("#ANDI508-scopeMode-button").attr("aria-pressed") === "true"){
                xANDI.showModeButtons("scope");
                AndiModule.activeActionButtons.scopeMode = true;
            }
            else{
                xANDI.showModeButtons("headersId");
                AndiModule.activeActionButtons.scopeMode = false;
            }
        }
        else{
            //No [headers], force scopeMode
            xANDI.hideModeButtons();
            AndiModule.activeActionButtons.scopeMode = true;
        }
...
xANDI.viewList_buildTable = function(){

    //Build scrollable container and table head
    var appendHTML = "<div id='xANDI508-viewList' class='ANDI508-viewOtherResults-expanded' style='display:none;'>"+
    ...
    //Build table body
    var tableName;
    for(var x=0; x<xANDI.dataTables.list.length; x++){
        appendHTML += "<tr";
        //Highlight the select table
        if($(xANDI.dataTables.list[x].elementList[0]).hasClass("ANDI508-element"))
            appendHTML += " class='ANDI508-table-row-inspecting' aria-selected='true'";

        tableName = preCalculateTableName(xANDI.dataTables.list[x].elementList[0]);
...
xANDI.viewList_attachEvents = function(){
    //Add focus click to each link (output) in the table
    $("#ANDI508-viewList-table td a").each(function(){
        andiLaser.createLaserTrigger($(this),$(xANDI.dataTables.list[$(this).attr("data-andi508-relatedtable")].elementList[0]));
    })
    .click(function(){//Jump to this table
        //Make this link appear selected
        xANDI.reset();
        activeTableIndex = $(this).attr("data-andi508-relatedtable");
        analyzeTable(xANDI.dataTables.list[activeTableIndex].elementList[0]);
        andiFocuser.focusByIndex(1);
        xANDI.redoMarkup();
        xANDI.viewList_highlightSelectedTable(activeTableIndex, false);
        andiResetter.resizeHeights();
        return false;
    });
};
...
xANDI.viewList_highlightSelectedTable = function(index, scrollIntoView){
...
xANDI.grab_headers = function(element, elementData, table){
    var headers = $.trim($(element).attr("headers"));
