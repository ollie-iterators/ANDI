//=========================================//
//wANDI: data table rows ANDI              //
//Created By Social Security Administration//
//=========================================//

function init_module(){

var wANDIVersionNumber = "11.2.1";
...
wANDI.analyze = function(objectClass){
...
AndiModule.inspect = function(element){
    if ($(element).hasClass("ANDI508-element")) {
        ...
        //Remove other wANDI highlights
        $("#ANDI508-testPage .wANDI508-highlight").removeClass("wANDI508-highlight");
        //Highlight This Element
        $(element).addClass("wANDI508-highlight");
    ...
    function grabHeadersAndHighlightRelatedCells(element){
        ...
        if(!AndiModule.activeActionButtons.scopeMode){
            ...
            if($(element).is("th")){
                ...
                if(id){
                    $(table).find("th.ANDI508-element:not(.wANDI508-highlight),td.ANDI508-element:not(.wANDI508-highlight)").each(function(){
        ...
        function addHighlight(element, updateAssociatedHeaderCellsText){
            ...
            if(updateAssociatedHeaderCellsText){
                ...
                if(accumulatedHeaderTextArray.indexOf(text) === -1){
                    ...
                    accumulatedHeaderText += text + wANDI.associatedHeaderCellsDelimeter;
                }
            }
        }
...
//This function will remove wANDI markup from every table and rebuild the alert list
wANDI.reset = function(){
    var testPage = document.getElementById("ANDI508-testPage");

    //Every ANDI508-element
    $(testPage).find(".ANDI508-element").each(function(){
        $(this)
            .removeClass("wANDI508-highlight")
...
wANDI.hideModeButtons = function(){
...
wANDI.showModeButtons = function(mode){
    AndiModule.activeActionButtons.modeButtonsVisible = true;
    ...
    if(role === "table" || ((role === "grid" || role === "treegrid") && $(table).find("[role=gridcell]").first().length)){
        ...
        if(tableHasHeaders){
            ...
            if(AndiModule.activeActionButtons.modeButtonsVisible && $("#ANDI508-scopeMode-button").attr("aria-pressed") === "true"){
                wANDI.showModeButtons("scope");
                AndiModule.activeActionButtons.scopeMode = true;
            }
            else{
                wANDI.showModeButtons("headersId");
                AndiModule.activeActionButtons.scopeMode = false;
            }
        }
        else{
            //No [headers], force scopeMode
            wANDI.hideModeButtons();
            AndiModule.activeActionButtons.scopeMode = true;
        }
...
wANDI.viewList_buildTable = function(){

    //Build scrollable container and table head
    var appendHTML = "<div id='wANDI508-viewList' class='ANDI508-viewOtherResults-expanded' style='display:none;'>"+
    ...
    //Build table body
    var tableName;
    for(var x=0; x<wANDI.dataTables.list.length; x++){
        appendHTML += "<tr";
        //Highlight the select table
        if($(wANDI.dataTables.list[x].elementList[0]).hasClass("ANDI508-element"))
            appendHTML += " class='ANDI508-table-row-inspecting' aria-selected='true'";

        tableName = preCalculateTableName(wANDI.dataTables.list[x].elementList[0]);
...
wANDI.viewList_attachEvents = function(){
    //Add focus click to each link (output) in the table
    $("#ANDI508-viewList-table td a").each(function(){
        andiLaser.createLaserTrigger($(this),$(wANDI.dataTables.list[$(this).attr("data-andi508-relatedtable")].elementList[0]));
    })
    .click(function(){//Jump to this table
        //Make this link appear selected
        wANDI.reset();
        activeTableIndex = $(this).attr("data-andi508-relatedtable");
        analyzeTable(wANDI.dataTables.list[activeTableIndex].elementList[0]);
        andiFocuser.focusByIndex(1);
        wANDI.redoMarkup();
        wANDI.viewList_highlightSelectedTable(activeTableIndex, false);
        andiResetter.resizeHeights();
        return false;
    });
};
...
wANDI.viewList_highlightSelectedTable = function(index, scrollIntoView){
