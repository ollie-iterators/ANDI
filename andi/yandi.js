...
//create yANDI instance
var yANDI = new AndiModule(yANDIVersionNumber,"y");

//Delimeter for the the header cells
yANDI.associatedHeaderCellsDelimeter = " <span aria-hidden='true'>|</span> ";
...
$("#ANDI508-button-prevElement").off("click").click(function(){
    ...
    if(isNaN(index)){ //no active element yet
        ...
        if(yANDI.dataTables.list.length <= 1)
...
yANDI.analyze = function(objectClass){
    ...
    $(TestPageData.allElements).each(function(){
        ...
        //Determine if this is a refresh of yANDI (there is an active element)
...
AndiModule.inspect = function(element){
    if ($(element).hasClass("ANDI508-element")) {
        ...
        //Remove other yANDI highlights
        $("#ANDI508-testPage .yANDI508-highlight").removeClass("yANDI508-highlight");
        //Highlight This Element
        $(element).addClass("yANDI508-highlight");
    ...
    function grabHeadersAndHighlightRelatedCells(element){
        ...
        if(!AndiModule.activeActionButtons.scopeMode){
            ...
            if($(element).is("th")){
                ...
                if(id){
                    $(table).find("th.ANDI508-element:not(.yANDI508-highlight),td.ANDI508-element:not(.yANDI508-highlight)").each(function(){
        ...
        function addHighlight(element, updateAssociatedHeaderCellsText){
            ...
            if(updateAssociatedHeaderCellsText){
                ...
                if(accumulatedHeaderTextArray.indexOf(text) === -1){
                    ...
                    accumulatedHeaderText += text + yANDI.associatedHeaderCellsDelimeter;
                }
            }
        }
...
//This function will remove yANDI markup from every table and rebuild the alert list
yANDI.reset = function(){
    var testPage = document.getElementById("ANDI508-testPage");

    //Every ANDI508-element
    $(testPage).find(".ANDI508-element").each(function(){
        $(this)
            .removeClass("yANDI508-highlight")
...
yANDI.hideModeButtons = function(){
...
yANDI.showModeButtons = function(mode){
    AndiModule.activeActionButtons.modeButtonsVisible = true;
...
function analyzeTable(table){
    ...
    if(role === "table" || ((role === "grid" || role === "treegrid") && $(table).find("[role=gridcell]").first().length)){
        ...
        //The way yANDI analyzes the table is that it begins looking at the cells first
        ...
        $(all_cells).each(function loopC(){
            ...
            if(headers)
                yANDI.grab_headers(cell, andiData, table);
        ...
        if(tableHasHeaders){
            ...
            if(AndiModule.activeActionButtons.modeButtonsVisible && $("#ANDI508-scopeMode-button").attr("aria-pressed") === "true"){
                yANDI.showModeButtons("scope");
                AndiModule.activeActionButtons.scopeMode = true;
            }
            else{
                yANDI.showModeButtons("headersId");
                AndiModule.activeActionButtons.scopeMode = false;
            }
        }
        else{
            //No [headers], force scopeMode
            yANDI.hideModeButtons();
            AndiModule.activeActionButtons.scopeMode = true;
        }
...
yANDI.viewList_buildTable = function(){

    //Build scrollable container and table head
    var appendHTML = "<div id='yANDI508-viewList' class='ANDI508-viewOtherResults-expanded' style='display:none;'>"+
    ...
    //Build table body
    var tableName;
    for(var x=0; x<yANDI.dataTables.list.length; x++){
        appendHTML += "<tr";
        //Highlight the select table
        if($(yANDI.dataTables.list[x].elementList[0]).hasClass("ANDI508-element"))
            appendHTML += " class='ANDI508-table-row-inspecting' aria-selected='true'";

        tableName = preCalculateTableName(yANDI.dataTables.list[x].elementList[0]);
...
yANDI.viewList_attachEvents = function(){
    //Add focus click to each link (output) in the table
    $("#ANDI508-viewList-table td a").each(function(){
        andiLaser.createLaserTrigger($(this),$(yANDI.dataTables.list[$(this).attr("data-andi508-relatedtable")].elementList[0]));
    })
...
yANDI.grab_headers = function(element, elementData, table){
    var headers = $.trim($(element).attr("headers"));
...
andiBar.results(yANDI.dataTables, [], showStartUpSummaryText);
