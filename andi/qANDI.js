...
function init_module() {
    ...
    qANDI.analyze = function () {
        ...
        $(TestPageData.allElements).each(function () {
            if ($(this).isSemantically("[role=heading]", "h1,h2,h3,h4,h5,h6")) {
                ...
                //Since qANDI has not found a heading yet, check if this element is a fake headings

                if (qANDI.isFakeHeading(this)) {
                    structureExists = true;
    ...
    qANDI.isFakeHeading = function (element) {
    ...
    qANDI.results = function () {
        ...
        moduleActionButtons += "<button id='ANDI508-headings-button' class='qANDI508-mode' aria-label='" + headingsArray.length + " Headings'>" + headingsArray.length + " headings</button>";
        moduleActionButtons += "<button id='ANDI508-lists-button' class='qANDI508-mode' aria-label='" + listsCount + " Lists'>" + listsCount + " lists</button>";
        moduleActionButtons += "<button id='ANDI508-landmarks-button' class='qANDI508-mode' aria-label='" + landmarksArray.length + " Landmarks'>" + landmarksArray.length + " landmarks</button>";
        moduleActionButtons += "<button id='ANDI508-liveRegions-button' class='qANDI508-mode' aria-label='" + liveRegionsArray.length + " Live Regions'>" + liveRegionsArray.length + " live regions</button>";
        ...
        //Define qANDI mode buttons (headings, lists, landmarks)
            ...
            if (AndiModule.activeActionButtons.headings) {
                ...
                if (headingsArray.length > 0) {
                    ...
                    for (var x = 0; x < headingsArray.length; x++) {
                        qANDI.outline += qANDI.getOutlineItem(headingsArray[x]);
                    }
                    qANDI.outline += "</div>";

                    $("#ANDI508-additionalPageResults").html("<button id='ANDI508-viewOutline-button' class='ANDI508-viewOtherResults-button' aria-expanded='false'>" + listIcon + "view headings list</button><div id='qANDI508-outline-container' class='ANDI508-viewOtherResults-expanded' tabindex='0'></div>");

                    //Define outline button
                    $("#ANDI508-viewOutline-button").click(function () {
                        if ($(this).attr("aria-expanded") === "true") {
                            //hide Outline, show alert list
                            $("#qANDI508-outline-container").slideUp(AndiSettings.andiAnimationSpeed);
                            ...
                            $("#qANDI508-outline-container").slideDown(AndiSettings.andiAnimationSpeed).focus();
                        }
                        andiResetter.resizeHeights();
                        return false;
                    });
