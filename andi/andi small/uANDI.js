...
function init_module() {
    ...
    $("#ANDI508-button-nextElement").off("click").click(function () {
        ...
        if (index == testPageData.andiElementIndex || isNaN(index)) {
            if (tableCountTotal <= 1) { //If there is only 1 table, loop back to first cell
    ...
    AndiModule.inspect = function (element) {
        ...
        function grabHeadersAndHighlightRelatedCells(element) {
            ...
            var rowIndex = $(element).attr("data-uandi508-rowindex");
            var colIndex = $(element).attr("data-uandi508-colindex");
            var colgroupIndex = $(element).attr("data-uandi508-colgroupindex");
            var rowgroupIndex = $(element).attr("data-uandi508-rowgroupindex");
            ...
            if (!AndiModule.activeActionButtons.scopeMode) {
                ...
                if ($(element).is("th")) {
                    ...
                    if (id) {
                        $(table).find("th.ANDI508-element:not(.uANDI508-highlight),td.ANDI508-element:not(.uANDI508-highlight)").filter(":visible").each(function () {
                ...
                if ($(element).is("td")) {
                    ...
                    $(table).find("th.ANDI508-element").filter(":visible").each(function () {
                        s = $(this).attr("scope");
                        ci = $(this).attr("data-uandi508-colindex");
                        ri = $(this).attr("data-uandi508-rowindex");

                        //get associated th from col
                        if (s != "row" && s != "rowgroup" &&
                            (!colgroupIndex || (colgroupIndex == $(this).attr("data-uandi508-colgroupindex"))) &&
                            index_match(colIndex, ci) && !index_match(rowIndex, ri)) {
                            addHighlight(this, true);
                            //get associated th from row
                        } else if (s != "col" && s != "colgroup" &&
                            (!rowgroupIndex || (rowgroupIndex == $(this).attr("data-uandi508-rowgroupindex"))) &&
                    ...
                    $(table).find("th.ANDI508-element,td.ANDI508-element").filter(":visible").each(function () {
                        ...
                        ci = $(this).attr("data-uandi508-colindex");
                        ri = $(this).attr("data-uandi508-rowindex");
                        cgi = $(this).attr("data-uandi508-colgroupindex");
                        rgi = $(this).attr("data-uandi508-rowgroupindex");
                        ...
                        if (scope) {
                            ...
                            if (isSameColgroup && scope === "col" && col_index_matches) {
                                ...
                                if ($(element).parent().attr("data-uandi508-colgroupsegment")) {
            ...
            function addHighlight(element, updateAssociatedHeaderCellsText) {
                ...
                if (updateAssociatedHeaderCellsText) {
                    ...
                    if (accumulatedHeaderTextArray.indexOf(text) === -1) {
                        ...
                        accumulatedHeaderText += text + " <span aria-hidden='true'>|</span> ";
                    }
                }
            }
            ...
            if (role === "presentation" || role === "none") {
                ...
                if (tableHasHeaders) {
                    ...
                    if (AndiModule.activeActionButtons.modeButtonsVisible && $("#ANDI508-scopeMode-button").attr("aria-pressed") === "true") {
                        uANDI.showModeButtons("scope");
                        AndiModule.activeActionButtons.scopeMode = true;
                    } else {
                        uANDI.showModeButtons("headersId");
                        AndiModule.activeActionButtons.scopeMode = false;
                    }
                } else { //No [headers], force scopeMode
                    uANDI.hideModeButtons();
                    AndiModule.activeActionButtons.scopeMode = true;
                }
